...
static float tof_convert_us_to_inches(uint32_t pulse_width_us) {
    return pulse_width_us / 1250.0f;
}
...
void tof_sensor_update(ToFSensor *tof) {
    if (tof == NULL || !tof->new_measurement) return;
